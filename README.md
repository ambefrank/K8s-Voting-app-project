...
.(second option) Or better still, go to your AWS Management Console, navigate to EC2 interface and open the AWS-CLI interface by clicking on the shell icon at the top right conner of your AWS portal.
...
mapRoles: |
  - rolearn: arn:aws:iam::XXXXXXXXXXXX:role/testrole
    ...
    groups:
      - system:masters
```
Now, return to your instance's CLI.
...
```
{
API_ELB_PUBLIC_FQDN=$(kubectl get svc api -n cloudchamp -o jsonpath="{.status.loadBalancer.ingress[0].hostname}")
...
```
{
FRONTEND_ELB_PUBLIC_FQDN=$(kubectl get svc frontend -n cloudchamp -o jsonpath="{.status.loadBalancer.ingress[0].hostname}")
...
https://youtu.be/pTmIoKUeU-A?si=W4K2Lj7taPfD8e-q
